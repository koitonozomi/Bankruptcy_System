...
function calculateAlertStatus(caseItem: Partial<CaseProgress>): AlertDetail {
    const today = dayjs().startOf('day');
    let highestAlert: AlertDetail = { status: null, stepName: null, delayDays: null };
    ...
    for (const condition of alertConditionsMasterList) {
        ...
        let currentStatus: '黒' | '赤' | '黄' | null = null;
        ...
        const alertLevels: { [key: string]: number } = { 
            '黄': 1, 
            '赤': 2, 
            '黒': 3, 
            'null': 0 
...
let alertConditionsMasterList: AlertCondition[] = [];

try {
  // ファイルを同期的に読み込む
  const fileContent = fs.readFileSync(alertConditionsPath, 'utf-8');
  // 読み込んだ内容をパースして配列に代入
  alertConditionsMasterList = JSON.parse(fileContent) as AlertCondition[];
  console.log('✅ アラート設定を "alert-conditions.json" から読み込みました。');
} catch (error) {
  console.error(
    `❌ "alert-conditions.json" の読み込みに失敗しました。ファイルが存在するか確認してください。`,
    error
  );
  ...
  const mappedCases = caseResult.rows.map(row => {
    ...
    if (row["詳細状況（聞取後）"]) {
      try {
        ...
        if (Array.isArray(parsed)) reminderDates = parsed;
      } catch {
// JSONが壊れてる場合のみログ出力
...
date_midterm_report_deadline: row.財産目録提出期限,
      ...
      return null;
     }).filter(c => c && c.client_name) as CaseProgress[]; // nullをフィルタリングし、型をキャスト
...
app.post('/api/alert-conditions', (req, res) => {
  try {
    ...
    alertConditionsMasterList.push(...(newConditions as AlertCondition[]));
    ...
    const currentAlerts = new Map<number, '黒' | '赤' | '黄'>();
     for (const row of result.rows) {
      ...
      if (alertStatus) {
        ...
        const currentLevel = alertLevels[alertStatus as '黒'|'赤'|'黄']; 
